...
impl MCPServer {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        eprintln!("超时时间: {}秒", self.config.timeout);
        eprintln!("过滤思考内容: {}", self.config.filter_thinking);
        eprintln!("日志级别: {}", self.config.log_level);
    ...
    fn handle_tools_list(&self) -> Value {
        let description = if self.config.max_query_plan == 1 {
            format!("使用 AI 模型 ({}) 进行网络搜索。直接搜索用户查询，返回详细的搜索结果。", self.config.model_id)
        } else {
            format!(
    ...
    async fn handle_tools_call(&self, params: Option<Value>) -> Result<Value> {
        ...
        eprintln!("搜索成功,返回 {} 字符", result.len());
